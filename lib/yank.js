...
function addToConfig(yank, name){
    if(config() == true){
        ...
        if(getYank(name) == undefined){
            ...
            console.log(chalk.red('Yank name "') + chalk.blue.bold(name) + chalk.red('" already exists! Remove the original or choose another name...'))
        }
        } else{
...
function yankGitRepo(url, name){
    ...
    if(fs.existsSync('./' + folder)){
        console.log(chalk.red('Directory with repository name "' + chalk.blueBright.bold(folder) + '" already exists. Remove the directory or yank the existing folder.'));
        return 0;
    } else{
        if (fs.existsSync(configPath)){
